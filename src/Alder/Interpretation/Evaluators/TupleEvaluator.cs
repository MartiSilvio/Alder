...
internal static class TupleEvaluator
{
    ...
    public static async ValueTask<object?> EvaluateAsync(BoundTupleExpr node, EvaluationContext ctx, CancellationToken ct)
    {
        var values = new object?[node.Elements.Length];
        for (var i = 0; i < node.Elements.Length; i++)
            values[i] = await ctx.EvaluateAsync(node.Elements[i], ct);
