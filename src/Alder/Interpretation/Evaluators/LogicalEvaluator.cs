...
internal static class LogicalEvaluator
{
    public static object? Evaluate(BoundLogicalExpr node, EvaluationContext ctx)
    {
        ...
        {
            return EvaluateNullableBoolLogical(left as bool?, node, ctx);
        }
    ...
    private static object? EvaluateNullableBoolLogical(bool? left, BoundLogicalExpr logical, EvaluationContext ctx)
    {
        if (logical.Operator == TokenType.AmpAmp)
        {
            if (left == false) return BoxedConstants.False;
            var right = ctx.Evaluate(logical.Right) as bool?;
    ...
    public static async ValueTask<object?> EvaluateAsync(BoundLogicalExpr node, EvaluationContext ctx)
    {
        ...
        if (node.Left.StaticType.ClrType == typeof(bool?) || node.Right.StaticType.ClrType == typeof(bool?))
        {
            return await EvaluateNullableBoolLogicalAsync(left as bool?, node, ctx);
        }
        ...
        var right = await ctx.EvaluateAsync(node.Right);
        if (right is not bool)
        {
            throw new AlderException(
                DiagnosticDescriptors.BadBinaryOps,
    ...
    private static async ValueTask<object?> EvaluateNullableBoolLogicalAsync(bool? left, BoundLogicalExpr logical, EvaluationContext ctx)
    {
        if (logical.Operator == TokenType.AmpAmp)
        {
            if (left == false) return BoxedConstants.False;
            var right = await ctx.EvaluateAsync(logical.Right) as bool?;
            if (left == true) return right;
            return right == false ? BoxedConstants.False : null;
        }
