...
{
    public static object? Evaluate(BoundTypedLambdaExpr node, EvaluationContext ctx, CancellationToken ct)
    {
        var parameters = node.Parameters;
        var body = node.Body;
        var config = ctx.Context.Config;
        ...
        return new CompiledLambdaValue(
            ...
            {
                var evaluator = new BoundEvaluator(closureCtx);
                var result = evaluator.Evaluate(body);
                return result is ControlFlowSignal signal ? signal.Value : result;
            } : null,
