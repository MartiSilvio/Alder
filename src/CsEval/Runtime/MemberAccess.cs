...
internal static class MemberAccess
{
    public static object? GetMember(object? obj, string name, CsEvalOptions options, bool nullSafe, CsEvalContext context)
    {
            ...
            if (context.Config.AotMetadata is { } aotStaticMeta && aotStaticMeta.TryGetValue(staticType, out var staticMetadata))
            {
                if (staticMetadata.TryGetStaticProperty(name, out var aotStaticValue))
                {
                    if (!options.Sandbox.AllowStaticPropertyRead)
                        throw new CsEvalSandboxException(DiagnosticDescriptors.SandboxAccessBlocked, "Static property", staticType.Name, name);
                ...
                if (staticMetadata.TryGetStaticField(name, out aotStaticValue))
                {
                    if (!options.Sandbox.AllowStaticFieldRead)
                        throw new CsEvalSandboxException(DiagnosticDescriptors.SandboxAccessBlocked, "Static field", staticType.Name, name);
        ...
        if (!options.Sandbox.AllowPropertyRead)
            throw new CsEvalSandboxException(DiagnosticDescriptors.SandboxAccessBlocked, "Property", "instance", name);
    ...
    public static void SetMember(object? obj, string name, object? value, CsEvalOptions options, CsEvalContext context)
    {
        ...
        if (!options.Sandbox.AllowPropertySet)
            throw new CsEvalSandboxException(DiagnosticDescriptors.SandboxPropertyAssignmentBlocked, name);
