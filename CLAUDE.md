...
- In test expressions and documentation examples, do NOT wrap multi-statement code in `{ }` blocks unless the parser requires statement mode for that construct. Braces are needed for: control flow (`if`, `for`, `while`, `foreach`, `switch`, `try/catch`), `lock`, `using`, empty statements (`;`), `Action`/delegate invocations, and fully-qualified typed variable declarations (`System.X y = ...`). Keyword-based typed declarations (`int x`, `byte a`, `int? b`, `string s`, etc.) do NOT need braces — only FQN types like `System.Collections.Generic.List<int>` do. For simple declarations + return, braces are NOT needed — write `"var x = 1; return x + 1;"` not `"{ var x = 1; return x + 1; }"`.
