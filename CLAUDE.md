...
- `.roslyn.csx` parity siblings are ONLY needed when CsEval behavior diverges or might diverge from Roslyn — i.e., when there is genuine uncertainty about whether CsEval matches real C#. Do NOT create `.roslyn.csx` files for straightforward behavior that obviously matches C# (like basic arithmetic, simple string operations, standard control flow). A regular engine test with a hardcoded expected value is fine when the expected result is unambiguous. Reserve parity tests for edge cases, tricky semantics, or areas where CsEval's implementation could plausibly differ from the runtime.
