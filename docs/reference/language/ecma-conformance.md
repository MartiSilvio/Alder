...
This page is the detailed conformance surface for Alder’s Standard mode.
It is written to be auditable. It lists what is in scope for Alder’s runtime input surface, and declares conformance at the level of ECMA-334 sections.

**Spec edition:** ECMA-334, 7th edition (December 2023). See [ECMA-334](/spec/ecma-334/).
**Last verified:** 2026-04-14.

## Scope and non-goals

Alder evaluates expressions and statement blocks from strings. It does not compile programs.

These C# areas are explicitly out of scope for Alder’s input surface:

| Area                    | Status       | Notes                                                                                      |
| ----------------------- | ------------ | ------------------------------------------------------------------------------------------ |
| Type declarations       | Out of scope | `class`, `struct`, `interface`, `enum`, `namespace` are not parsed as declarations.        |
| Attributes              | Out of scope | Attribute syntax is not part of the input surface.                                         |
| Member declarations     | Out of scope | Methods, properties, fields, events, and access modifiers are not declared in Alder input. |
| Preprocessor directives | Out of scope | No `#if`, `#define`, etc.                                                                  |

Extended mode adds additional syntax that is not part of ECMA-334. See [Extended mode](/reference/language/extended/).
...
| Notes        | Use for constraints, edge cases, and clarifications.  |
...
| §6.4.4       | Contextual keywords                      | Supported | Contextual keywords are interpreted in context during parsing.                                |
| §6.4.5.3     | Integer literal type and promotion rules | Supported |                                                                                               |
| §6.4.5.4     | Real literals that start with `.`        | Supported |                                                                                               |
| §6.4.5.5     | Character literals                       | Supported | Includes escape sequences supported by the lexer.                                             |

## Variables (Chapter 9)

| ECMA section | Area     | Status    | Notes                                                                              |
| ------------ | -------- | --------- | ---------------------------------------------------------------------------------- |
| §9.2.9.1     | Discards | Supported | `_` is treated as a discard when not defined as a variable in the current context. |

## Conversions (Chapter 10)

| ECMA section | Area                                     | Status    | Notes                                                                                                           |
| ------------ | ---------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------------- |
| §10.2        | Implicit conversions                     | Supported | Alder implements the standard implicit conversion lattice used during binding.                                  |
| §10.2.3      | Implicit numeric conversions             | Supported |                                                                                                                 |
| §10.2.8      | Implicit reference conversions           | Supported | Includes reference and boxing conversions that apply in Alder’s bind-time conversion lattice.                   |
...
| §10.3        | Explicit conversions                     | Supported | Explicit conversions are validated at bind time and executed at runtime.                                        |
...
| §10.3.5      | Explicit reference conversions           | Supported | Validated at bind time; success depends on the runtime type.                                                    |
...
| §10.3.8      | Explicit dynamic conversions             | Supported | `dynamic` is treated as `object` in the Alder type system and participates in explicit conversions accordingly. |
| §10.4.2      | Standard implicit conversions            | Supported |                                                                                                                 |
| §10.5.x      | User-defined conversions                 | Supported | Conversions are discovered on the relevant types at bind time.                                                  |
| §10.5.3      | Evaluation of user-defined conversions   | Supported | User-defined conversion operators are evaluated at runtime after overload resolution.                           |
| §10.5.4      | User-defined implicit conversions        | Supported |                                                                                                                 |
| §10.5.5      | User-defined explicit conversions        | Supported |                                                                                                                 |
| §10.7.1      | General                                  | Supported | Nullable conversion lifting is applied where relevant.                                                          |
| §10.8        | Method group conversions                 | Supported | Method groups can be converted to compatible delegate types during binding.                                     |

## Patterns and pattern matching (Chapter 11)

ECMA-334 (7th edition) specifies declaration, constant, and var patterns.
Additional pattern forms implemented by Alder are documented elsewhere and are not treated as ECMA conformance.
...
| §12.4.8      | Lifted operators                                                    | Supported    | Nullable operator lifting follows the lifted-operator rules for predefined operators.                                   |
...
| §12.8.15     | Postfix increment and decrement operators                           | Supported    | `++` and `--` require a writable operand (variable, property, or indexer).                                              |
...
| §12.8.16.6   | Delegate creation expressions                                       | Supported    | Delegate creation via `new D(...)` is supported when `D` is a delegate type.                                            |
| §12.8.17     | `typeof`                                                            | Supported    |                                                                                                                         |
| §12.8.20     | Default value expressions                                           | Supported    |                                                                                                                         |
| §12.8.21     | Stack allocation                                                    | Out of scope | `stackalloc` is lexed, but stackalloc expressions are not currently parsed, bound, or evaluated as part of Alder input. |
...
| §12.9.8      | Await expressions                                                   | Supported    | Requires an async evaluation API surface.                                                                               |
...
| §12.20.2     | Ambiguities in query expressions                                    | Supported    | Query parsing follows the query-expression disambiguation rules.                                                        |
...
| §13.6.2         | Local variable declarations                            | Supported | Includes multi-declarators (`int x = 1, y = 2;`).                           |
...
| §13.9.4         | The for statement                                      | Supported | Includes empty statement bodies (`for (...);`).                             |
...
| ECMA section | Area         | Status    | Notes                                                                                             |
| ------------ | ------------ | --------- | ------------------------------------------------------------------------------------------------- |
| §7.3         | Declarations | Supported | Tuple element names are treated as compile-time metadata and do not affect runtime type identity. |
...
| ECMA section | Area              | Status    | Notes                                                                                                     |
| ------------ | ----------------- | --------- | --------------------------------------------------------------------------------------------------------- |
| §8.1         | General           | Supported | Alder’s Standard mode uses the CLR type system and reflection for type identity and member discovery.     |
| §8.2.4       | The dynamic type  | Supported | `dynamic` is supported and is treated as `object` for binding, producing runtime binding where necessary. |
| §8.2.6       | Interface types   | Supported | Interfaces are supported in type resolution and in conversion classification.                             |
...
| ECMA section | Area    | Status    | Notes                                                                     |
| ------------ | ------- | --------- | ------------------------------------------------------------------------- |
| §20.1        | General | Supported | Delegate invocation is implemented via the delegate’s `Invoke` signature. |
