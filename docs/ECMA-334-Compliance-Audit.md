...
Per ECMA §12.4.7.2 (Unary numeric promotions) and §12.4.7.3 (Binary numeric promotions):
...
**ECMA §12.12.3 - Floating-Point NaN Handling:**
...
Per ECMA §12.13.4, bool `&` returns bool. But C# allows `int & int` where the int came from implicit conversion of bool → int (via 0/1).
...
**ECMA §10.2.3 - Implicit Numeric Conversions Table:**
```
...
✅ **Verified:** `TypeHelpers.ImplicitConversions` dictionary matches ECMA §10.2.3 exactly (lines 62-74).
...
**ECMA §12.4.7.3 - Promotion Rules (in order):**
...
**ECMA §12.4.8 - Lifted Operator Rules:**
...
**ECMA §12.13.5 - bool? & and | Truth Table:**
...
```csharp
// ECMA §12.4.7.2: char promoted to int
...
```csharp
// ECMA §10.3.7: Unboxing requires exact type match
[Test]
