---
title: "Binder"
description: "Semantic analysis — per-node strategy, resolved vs dynamic nodes, BoundType, member and call resolution"
sidebar:
  order: 3
---
...
The binder uses a per-node strategy pattern. Each AST node type is handled by a dedicated static class in `Binding/Binders/`, annotated with `[BindsNode(typeof(XxxExpr))]`. There are ~75 such binder classes. Dispatch from AST node type to the correct binder is source-generated by `BinderDispatchGenerator` — the generated code maps each `Expr` subclass to its binder's static `Bind` method. This keeps each binder focused on one construct's semantics.
...
After binding, the bound tree passes through `BoundTreeOptimizer.Optimize()`:

1. **ConstantFoldingPass**: Evaluates compile-time constant subexpressions (binary, unary, logical, conditional) and replaces them with literals. Respects ECMA-334 constant expression rules — `uint + non_negative_int_constant` stays `uint` rather than promoting to `long`. Silently ignores fold failures (e.g., overflow) to preserve the original expression.

2. **DeadBranchEliminationPass**: Removes unreachable `if` branches when the condition has been folded to a constant `bool`. `if (false) { ... }` with no else becomes a void no-op. Runs after constant folding so conditions like `if (1 > 0)` are already reduced.

3. **ConversionInsertionPass** (compilation only): Inserts explicit `BoundCastExpr` nodes for binary operands with mismatched numeric types. The interpreter handles promotion at runtime via `NumericDispatch.PromoteOperands`, but LINQ expression trees require exact type matching. Skips strict equality (`===`, `!==`) and `object`-typed operands.
