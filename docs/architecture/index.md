...
**Signal-based control flow.** `break`, `continue`, `return`, and `goto` produce `ControlFlowSignal` values that propagate through the evaluation stack as ordinary return values — not exceptions. This keeps the fast path clean and makes control flow composable across nested constructs.
