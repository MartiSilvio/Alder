...
Extended mode is Alder's opt-in language superset. It adds operators, syntax sugar, built-in functions, and convenience features inspired by Python, Ruby, Perl, SQL, and Rust.
...
```csharp
var options = AlderOptions.Default with { LanguageMode = LanguageMode.Extended };
...
If you are embedding Alder as a rule engine or calculator, Extended mode lets your users write `price between 10 and 100` instead of `price >= 10 && price <= 100`. If you are building a data pipeline, `values |> sum` reads more naturally than calling aggregate functions directly.
...
Attempting an Extended-only feature in Standard mode throws `AlderLanguageModeException` with a message identifying the feature:

```csharp
var engine = new AlderEngine(); // Standard mode (default)

engine.Evaluate("2 ** 10");
// throws AlderLanguageModeException: "Feature '**' requires Extended mode"
```
