...
Date functions respect `AlderOptions.IsCaseSensitive`. When case-insensitive, `Now()` and `TODAY()` also work.
...
Aggregate builtins respect `AlderOptions.IsCaseSensitive`. When case-insensitive, `SUM`, `AVG`, etc. also work.
