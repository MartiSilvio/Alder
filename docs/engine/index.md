Alder's public API is built around `AlderEngine`, configured once at construction, immutable after that, thread-safe for concurrent evaluation. From a single engine instance: evaluate C# code, parse and reuse expressions, validate syntax and semantics without executing, compile to native IL delegates, trace evaluation step-by-step, inject variables from your application.

Configuration covers language mode, security policy, execution limits, type registration, custom functions, class-backed modules, and AOT metadata via `AlderOptions` and its sub-builders.
...
| [Async/Await](async.md) | EvaluateAsync, awaitable types, async lambdas, CancellationToken injection |
