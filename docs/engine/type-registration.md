...
description: "Make host types available for construction, static access, and type references in CsEval expressions."
sidebar:
  order: 5
---
...
By default, CsEval expressions can use built-in type keywords (`int`, `string`, `bool`, etc.) and common BCL types (`List`, `Dictionary`, `Task`, etc.) without any registration. To use your own types or types from other assemblies, you register them with the engine.
...
public CsEvalEngine RegisterExtensionMethods<T>()
```
...
When an expression references a type name, CsEval resolves it in this order:
...
`SandboxOptions.AllowedTypes` provides a type-level allowlist. When set, only the listed types can be used for construction and static member access in expressions. Types not in the set trigger a `CsEvalSandboxException`.
...
// StringBuilder is NOT in AllowedTypes -- throws CsEvalSandboxException
...
Registers an AOT-compatible type context generated by the CsEval source generator. This enables type metadata to be available without runtime reflection, which is required for Native AOT deployment.
...
The built-in context (`CsEvalBuiltInContext.Default`) is registered automatically. Use `UseGeneratedContext` to add metadata for your own types.
