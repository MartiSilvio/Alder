...
Alder evaluates user-supplied C# code safely in production environments — multi-tenant SaaS, rule engines processing untrusted formulas, configuration-driven business logic, interactive REPLs. The security model provides three layers of control:

**Operation permissions** — eight flags controlling what evaluated code can do: call methods, read properties, assign variables, construct objects, access static members, write to indexers. Three presets (`Trusted`, `Safe`, `Strict`) cover common scenarios, with full customization via `with` expressions on the `SandboxOptions` record.
