...
|   ✅   | Hex literals      | `0xFF`, `0x1A`               | ✅  | ✅  | Per ECMA-334 §6.4.5.3: int → uint → long → ulong |
|   ✅   | Binary literals   | `0b1010`                     | ✅  | ✅  | Per ECMA-334 §6.4.5.3: int → uint → long → ulong |
...
|   ✅   | Leading decimal   | `.5`, `.123`                 | ✅  | ✅  | ECMA-334 §6.4.5.4                                |
...
|   🟡   | `goto case/default` | `goto case 1;`, `goto default;`         | ❌  | ❌  | ECMA-334 §13.10.4             |
...
|   ✅   | Type casting          | `(int)x`                      | ✅  | ✅  |                                                                         |
|   ✅   | `nameof`              | `nameof(property)`            | ✅  | ✅  | ECMA-334 §12.8.22                                                       |
|   ✅   | `default`             | `default(int)`                | ✅  | ✅  | ECMA-334 §12.8.20                                                       |
...
| :----------- | :---------------- | :--------: | :----------------------------------------------------------------------------- |
| §6           | Lexical Structure |    98%     | Missing: `@keyword` verbatim identifiers                                       |
...
| §11          | Patterns          |    75%     | Full type/var patterns; no property/relational/logical                         |
| §12          | Expressions       |    95%     | Full operators; default/nameof implemented; no typeof/tuples                   |
...
| Leading decimal (`.5`)                          | §6.4.5.4     |   ✅   | Supported                                                        |
...
| Var pattern (`x is var y`)                          | §11.2.4      |   ✅   | Always matches, binds variable          |
...
| Null-conditional element (`arr?[i]`)          | §12.8.12     |   ✅   | Full support           |
...
| `default`                                     | §12.8.20     |   ✅   | Value and ref types    |
| `nameof`                                      | §12.8.22     |   ✅   | Simple and qualified   |
...
2. ~~✅ Char arithmetic (`'A' + 1` => 66, `'B' - 'A'` => 1) - ECMA-334 §12.10.2 - Fixed~~
3. 🟡 Property patterns (`x is { Name: "John" }`)

**Low Priority (Rarely Used):**

1. 🔵 Verbatim identifiers (`@if`, `@class`)
2. 🔵 Switch case guards (`when`)
...
10. ~~**Unboxing Bug**: Fixed - now correctly throws `InvalidCastException` when unboxing to wrong type per ECMA-334 §10.3.7.~~
...
~~13. **Hex/Binary Literal Type Inference Incomplete**: Per ECMA-334 §6.4.5.3, unsuffixed hex/binary literals should be inferred to the smallest fitting type (int → uint → long → ulong).~~ ✅ Fixed - Proper type promotion chain implemented, including int.MinValue and long.MinValue edge cases.
