...
public class LambdaOverloadResolutionTests(CompilationMode mode)
{
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 1, 2, 3 }.Sum(x => x * 2)");
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 1, 2, 3 }.Sum(x => x * 1.0)");
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 3, 4 }.Sum(x => Math.Pow(x, 2))");
        Assert.That(result, Is.TypeOf<double>());
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 10, 20, 30 }.Sum(x => x / 2.0)");
        Assert.That(result, Is.TypeOf<double>());
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 1, 2, 3 }.Sum(x => (long)x)");
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 1, 2, 3 }.Sum(x => (float)x)");
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 1, 2, 3 }.Sum(x => (decimal)x)");
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 2, 4, 6 }.Average(x => x)");
        Assert.That(result, Is.TypeOf<double>());
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 2, 4, 6 }.Average(x => x * 1.0)");
        Assert.That(result, Is.TypeOf<double>());
        Assert.That(result, Is.EqualTo(4.0));
    }
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 1, 2, 3 }.Select(x => x * 2.0).Sum()");
        Assert.That(result, Is.TypeOf<double>());
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 1, 2, 3, 4, 5 }.Where(x => x > 2).Sum(x => x * 0.5)");
        Assert.That(result, Is.TypeOf<double>());
        Assert.That(result, Is.EqualTo(6.0));
    }
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new int[0].Sum(x => x * 2.0)");
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 42 }.Sum(x => Math.Sqrt(x))");
        Assert.That(result, Is.TypeOf<double>());
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 2, 8 }.Sum(x => Math.Log(Math.Pow(x, 2)))");
        Assert.That(result, Is.TypeOf<double>());
    }
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("new[] { 1, 2, 3 }.Sum(x => x > 1 ? x * 1.5 : 0.0)");
        Assert.That(result, Is.TypeOf<double>());
    ...
    {
        var result = TestEngineFactory.Create(mode).Evaluate("""new[] { "hello", "world" }.Sum(s => s.Length)""");
