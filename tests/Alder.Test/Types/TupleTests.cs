...
public class TupleTests(CompilationMode mode)
{
    ...
    public void TupleConversion_IsTupleType_DetectsTupleTypes()
    {
        ...
        Assert.That(Alder.Runtime.TypeHelpers.IsTupleType(typeof(ValueTuple<int, int>)), Is.True);
        Assert.That(Alder.Runtime.TypeHelpers.IsTupleType(typeof(ValueTuple<int, string>)), Is.True);
        Assert.That(Alder.Runtime.TypeHelpers.IsTupleType(typeof(ValueTuple<int, long, double>)), Is.True);
        Assert.That(Alder.Runtime.TypeHelpers.IsTupleType(typeof(int)), Is.False);
        Assert.That(Alder.Runtime.TypeHelpers.IsTupleType(typeof(string)), Is.False);
    }
    ...
    {
        // Engine-only: tests TypeHelpers internal API directly
        Assert.That(Alder.Runtime.TypeHelpers.CanImplicitlyConvert(
    ...
    public void Tuple_RuntimeType_IsValueTuple()
    {
        ...
        var result = engine.Evaluate("(1, 2)");
        Assert.That(result, Is.Not.Null);
        Assert.That(Alder.Runtime.TypeHelpers.IsTupleType(result!.GetType()), Is.True);
    }
