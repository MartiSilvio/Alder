...
/// Validates that Alder's conversion behavior matches Roslyn and the ECMA-334 7th edition spec exactly.
///
...
public class ConversionTests(CompilationMode mode)
{
    ...
    {
        // Engine-only: error assertion (both Alder and Roslyn must throw)
        var engine = TestEngineFactory.Create(mode);
