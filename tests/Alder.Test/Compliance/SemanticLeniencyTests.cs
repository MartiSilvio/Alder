...
/// <summary>
/// Verifies Alder Standard mode matches Roslyn across 6 semantic areas:
...
public class SemanticLeniencyTests(CompilationMode mode)
{
    ...
    private async Task AssertMatchesRoslyn(string expr)
    {
        ...
        var alderResult = engine.Evaluate(expr);

        Assert.That(alderResult, Is.EqualTo(roslynResult),
            $"Value mismatch for: {expr}\n  Alder={alderResult} ({alderResult?.GetType()?.Name})\n  Roslyn={roslynResult} ({roslynResult?.GetType()?.Name})");
        Assert.That(alderResult?.GetType(), Is.EqualTo(roslynResult?.GetType()),
            $"Type mismatch for: {expr}\n  Alder type={alderResult?.GetType()?.Name}\n  Roslyn type={roslynResult?.GetType()?.Name}");
    }
    ...
    private async Task AssertBothThrow(string expr)
    {
        ...
        Assert.Catch<Exception>(() => engine.Evaluate(expr),
            $"Alder accepted but Roslyn rejected: {expr}");
    }
