...
/// <summary>
/// Empirically verifies C# 12 feature support in Roslyn Scripting and Alder.
...
public class CSharp12FeatureInventoryTests
{
    ...
    public async Task CollectionExpressions_RoslynScripting()
    {
        ...
        // Alder: Supported in Extended mode only (as array literal, not C# 12 collection expression)
        // Classification: Intentional difference -- Alder's [...] is Extended syntax
    }
    ...
    // Out of scope for an runtime evaluator -- class declarations are not expressions.
    ...
    public async Task PrimaryConstructors_RoslynScripting()
    {
        ...
        // Classification: Out of scope -- Alder evaluates expressions, not class declarations
    }
    ...
    [Test]
    public void LambdaDefaultParameters_Alder()
    {
        var engine = new AlderEngine(Options);
        try
        {
            var result = engine.Evaluate("((int x = 5) => x + 1)()");
            Assert.Pass($"Alder supports lambda default parameters: result = {result}");
        }
        catch (Exception ex)
        {
            Assert.Pass($"Alder does not support lambda default parameters: {ex.GetType().Name}");
        }
        // Classification: Potential Alder gap if Roslyn supports it
    }
    ...
    public async Task RefReadonlyParameters_RoslynScripting()
    {
        ...
        // Classification: Out of scope -- Alder does not support method declarations
    }

    // 5. Inline Arrays
    // Requires struct declaration with [InlineArray] attribute -- out of scope for runtime evaluator.
    ...
    // Requires using directive -- out of scope for runtime evaluator.
