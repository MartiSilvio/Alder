// All tests engine-only: Constraints config, AlderExecutionLimitException assertions
// -- Alder-specific resource constraint API with no Roslyn equivalent.
...
public class ResourceConstraintTests(CompilationMode mode)
{
    ...
    [Test]
    public void StatementException_IsCatchableAsAlderException()
    {
        var engine = CreateEngine(maxStatements: 3, mode: mode);
        // AlderExecutionLimitException inherits from AlderException
        Assert.Catch<AlderException>(
            () => engine.Evaluate("{ var x = 0; while (true) { x = x + 1; } return x; }"));
    }
