...
/// <summary>
/// Pre-release adversarial audit: Section 4 — Security Sandbox Escapes.
...
public class SecuritySandboxAuditTests(CompilationMode mode)
{
    ...
    // DEFECT-SEC-1: SecurityValidationPass does not validate BoundCastExpr target types.
    //
    ...
    // DEFECT-SEC-2: BoundDynamicCallExpr bypasses AllowMethodCalls check.
    //
    ...
    // DEFECT-SEC-3: MemberAccess.GetMember's Type handler (case Type staticType) lacks
    ...
    // DEFECT-SEC-4: System.Threading namespace is not fully denied.
    //
    // The default deny list blocks Thread, ThreadPool, Mutex, Semaphore, Timer as individual
    // types, but other System.Threading types like ManualResetEvent, AutoResetEvent, Barrier,
    // SpinLock, Interlocked are NOT blocked. ManualResetEvent.WaitOne can block the evaluation
    // thread indefinitely.
    //
    // Fix: Add "System.Threading" to DefaultDeniedNamespaces and explicitly trust
    // System.Threading.Tasks for async/await support.
    [Test]
    public void Attack_ManualResetEvent_ShouldBeBlocked()
    {
        // Default sandbox (Trusted) with no custom deny list.
        // ManualResetEvent is not in the default deny list — document this gap.
        var engine = TestEngineFactory.Create(mode);

        try
        {
            // If this succeeds, ManualResetEvent is accessible and could block a thread
            engine.Evaluate("return new System.Threading.ManualResetEvent(false);");
            Assert.Warn("ManualResetEvent is NOT in the default deny list — " +
                        "user expressions can block the evaluation thread with WaitOne()");
        }
        catch (AlderException)
        {
            Assert.Pass("ManualResetEvent is properly blocked");
        }
    }

    // --- Compiled path bypass test ---

    // DEFECT-SEC-5: If the binder checks security but compiled IL doesn't re-check,
