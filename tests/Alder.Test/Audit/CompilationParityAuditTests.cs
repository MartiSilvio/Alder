...
/// <summary>
/// Pre-release adversarial audit: Section 2 — Compilation Path Parity.
...
public class CompilationParityAuditTests(CompilationMode mode)
{
    private AlderEngine CreateEngine(Action<AlderOptions>? configure = null)
        => TestEngineFactory.Create(mode, configure);

    // DEFECT-CP-1: CastEmitter allows bool<->arithmetic casts that the interpreter rejects.
    //
    ...
    // DEFECT-CP-2: BinaryEvaluator NaN short-circuit returns bool instead of NaN for arithmetic.
    //
