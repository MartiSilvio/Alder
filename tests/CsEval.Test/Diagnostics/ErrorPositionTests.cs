...
{
    private static CsEvalEngine Interpreted() => new(CsEvalOptions.Default);

    [Test]
    public void UndefinedVariable_ReportsCorrectPosition()
    {
        var ex = Assert.Catch<CsEvalException>(() => Interpreted().Evaluate("1 + xyz"));
    ...
    {
        var ex = Assert.Catch<CsEvalException>(() => Interpreted().Evaluate("xyz + 1"));
        Assert.That(ex!.Line, Is.EqualTo(1));
    ...
    {
        var ex = Assert.Catch<CsEvalException>(() => Interpreted().Evaluate("1 + true"));
        Assert.That(ex!.Line, Is.EqualTo(1));
        Assert.That(ex.Column, Is.EqualTo(1));
        Assert.That(ex.Span.Start, Is.EqualTo(0));
