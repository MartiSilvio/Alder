...
/// through DiagnosticDescriptors to CsEvalException throw sites.
///
/// Each test triggers a specific error via the public CsEvalEngine API, then asserts:
...
public class DiagnosticCodeTests
{
    ...
    {
        var ex = Assert.Throws<CsEvalException>(() => _engine.Evaluate("42[0]"));
    ...
    {
        var ex = Assert.Throws<CsEvalException>(() => _engine.Evaluate("{ string s = 42; }"));
    ...
    {
        var ex = Assert.Throws<CsEvalException>(() => _engine.Evaluate("(string)42"));
    ...
    {
        var ex = Assert.Throws<CsEvalException>(() => _engine.Evaluate("(int)null"));
    ...
    {
        var ex = Assert.Throws<CsEvalException>(() => _engine.Evaluate("undefinedVar"));
    ...
    public void CS0104_AmbiguousReference_DirectConstruction()
    {
        ...
        var ex = new CsEvalException(DiagnosticDescriptors.AmbiguousReference,
    ...
    {
        var ex = Assert.Throws<CsEvalException>(() => _engine.Evaluate("{ int x = 1; int x = 2; }"));
    ...
    public void CS0139_BreakOrContinueOutsideLoop()
    {
        ...
        var ex = new CsEvalException(DiagnosticDescriptors.BreakOrContinueOutsideLoop);
    ...
    {
        var ex = Assert.Throws<CsEvalException>(() => _engine.Evaluate("{ throw; }"));
    ...
    {
        var ex = Assert.Throws<CsEvalException>(() => _engine.Evaluate("new NonExistentType()"));
    ...
    [Test]
    public void BackwardCompat_CatchCsEvalException_CatchesAllErrors()
    {
        // Proves that catch(CsEvalException) catches diagnostic-aware exceptions
        Assert.Throws<CsEvalException>(() => _engine.Evaluate("undefinedVar"));
    }
